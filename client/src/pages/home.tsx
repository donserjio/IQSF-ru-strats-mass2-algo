...
const STRATEGIES: Record<string, StrategyConfig> = {
  basket50: {
    ...
    archDesc: "The system combines 15 independent algorithms on BTC and ETH. Trading is conducted both long and short. Diversification по стратегиям обеспечивает стабильность в любых рыночных условиях — от трендов до боковиков.",
    ...
    execDesc: "Fully automated execution 24/7. The algorithm selects order type based on liquidity and spread. No human involvement eliminates the emotional factor.",
          ...
          <div className="text-left">
            ...
            <AnimatedSection delay={300}>
              <div className="flex flex-nowrap gap-3 w-full sm:w-auto">
                ...
                >
                  Results <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            </AnimatedSection>
          </div>
          ...
          <div>
            ...
            <AnimatedSection delay={500}>
              <div className="grid grid-cols-3 divide-x divide-border/20 rounded-xl border border-border/30 bg-card/30 backdrop-blur-sm mt-4">
                {[
                  { label: "Annual Return", value: getMetricValue(stats?.metrics, "CAGR", "—") },
                  { label: "Sharpe Ratio", value: getMetricValue(stats?.metrics, "Sharpe", "—") },
  ...
  return (
    <section id="monthly-returns" className="py-12 px-4 sm:px-6 relative" data-testid="section-monthly-returns">
      <div className="max-w-7xl mx-auto">
        <AnimatedSection>
          <div className="text-center mb-10">
            <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-3">Monthly Returns</h2>
            <p className="text-muted-foreground text-sm max-w-lg mx-auto">Compounded monthly performance breakdown by year</p>
            <LiveDataBadge text="Updated daily · Binance API" pulse={false} />
          </div>
        </AnimatedSection>
        <AnimatedSection delay={100}>
              ...
              <table className="min-w-[600px] w-full text-xs sm:text-sm font-mono">
                <thead>
                  <tr className="border-b border-border/30">
                    <th className="py-2 px-2 text-left text-cyan-400 font-semibold">Year</th>
                    {MONTH_LABELS_EN.map((m) => (
                      <th key={m} className="py-2 px-1.5 text-center text-cyan-400 font-semibold">{m}</th>
                    ))}
                    <th className="py-2 px-2 text-center text-cyan-400 font-semibold border-l border-cyan-500/20 bg-cyan-500/5">Total</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <BarChart data={chartBarData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}
                      ...
                      <Tooltip
                        content={({ active, payload, label }: any) => {
                          ...
                          const dateStr = new Date(label + "T00:00:00").toLocaleDateString("en-US", { day: "numeric", month: "long", year: "numeric" });
                          ...
                          return (
                            <div className="bg-card border border-border rounded-lg px-4 py-3 shadow-xl min-w-[200px]">
                              ...
                              <ChartLiveBadge text="Live trading data · Real account · Updated daily via API" />
                            </div>
                          );
                        }}
                        />
...
const ACCESS_TERMS_BASE = [
  ...
  { label: "Connection Format", value: "Exchange API key" },
  { label: "Trading Assets", value: "10 trading pairs, 5 approaches" },
  { label: "Collateral", value: "USDT" },
  { label: "Exchanges", value: "Binance, OKX, Bybit, Bitget, BingX" },
];
...
function buildFaqItems(sc: StrategyConfig) {
  return [
    ...
    },
    {
      ...
      a: "The data presented on the website is based on live algorithmic trading results. Past performance is not indicative of future results.",
    },
  ];
}
      ...
      <section className="py-12 px-4 sm:px-6 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Button
              size="lg"
              className="bg-gradient-to-r from-cyan-600/80 to-blue-700/80 text-white shadow-lg shadow-cyan-500/15 w-[260px] cta-pulse transition-all"
              onClick={() => window.open("https://t.me/etheremax", "_blank")}
            >
              <Send className="w-4 h-4 mr-2" />
